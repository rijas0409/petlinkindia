...
const HealthSafetySection = ({ petId, vaccinated }: HealthSafetySectionProps) => {
  ...
  // Hide section if pet is not vaccinated or no vaccination records exist
  ...
  const handleDownload = async () => {
    if (!latestVaccination.certificate_url) return;
    try {
      const { data } = await supabase.storage
        .from("pet-documents")
        .createSignedUrl(latestVaccination.certificate_url, 300);
      if (data?.signedUrl) {
        window.open(data.signedUrl, "_blank");
      }
    } catch {
      // Try as public URL fallback
      window.open(latestVaccination.certificate_url, "_blank");
    }
  };
